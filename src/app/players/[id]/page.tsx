                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col gap-0.5 mb-0.5">
                      ...
                      <div className="flex items-center gap-1 flex-nowrap">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border whitespace-nowrap shrink-0 ${r.teamLabel ? "text-orange-300 border-orange-400/30" : "text-gray-600 border-white/10"}`}>
                          {r.teamLabel || "미배정"}
                        </span>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border whitespace-nowrap shrink-0 ${r.jerseyNumber ? "text-blue-300 border-blue-400/30" : "text-gray-600 border-white/10"}`}>
                          {r.jerseyNumber ? `등번호:${r.jerseyNumber}` : "번호미배정"}
                        </span>
                        {r.isMVP && (
                          <span className="text-[10px] font-black px-1.5 py-0.5 rounded-full border text-yellow-300 border-yellow-400/40 bg-yellow-400/10 whitespace-nowrap shrink-0">⭐ MVP</span>
                        )}
                        {r.isFairplay && (
                          <span className="text-[10px] font-black px-1.5 py-0.5 rounded-full border text-blue-300 border-blue-400/40 bg-blue-400/10 whitespace-nowrap shrink-0">🤝 페어플레이</span>
                        )}
                      </div>
                    </div>
                    <p className="text-sm font-bold truncate">{r.schedule.title}</p>
