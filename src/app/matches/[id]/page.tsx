              ...
              <div className="space-y-3">
                {myPlayers.length > 0 ? (
                  <>
                    ...
                    {/* 선택된 선수가 GK면 무료 배지 */}
                    {(() => {
                      const selPlayer = myPlayers.find((p) => p.id === selectedPlayer);
                      const posMap: Record<string, string> = { 골키퍼: "GK", 수비수: "DF", 미드필더: "MF", 공격수: "FW" };
                      const posCode = selPlayer?.position ? (posMap[selPlayer.position] ?? selPlayer.position) : null;
                      const isFull = posCode && schedule.positionStatus?.[posCode]?.full;
                      return (
                        <>
                          {selPlayer?.position === "골키퍼" && (
                            <div className="text-xs text-green-400 bg-green-400/5 border border-green-400/15 rounded-lg px-3 py-2 font-bold">
                              GK 포지션 — 무료 참가 적용
                            </div>
                          )}
                          {isFull && (
                            <div className="text-xs text-red-400 bg-red-400/5 border border-red-400/15 rounded-lg px-3 py-2 font-bold">
                              {selPlayer?.position}({posCode}) 자리가 모두 찼습니다.
                            </div>
                          )}
                          ...
                          <button
                            ...
                            className="w-full bg-green-400 text-black font-black py-3 rounded-xl hover:bg-green-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                          >
                            {submitting
                              ? "신청 중..."
                              : isFull
                                ...
                                : selPlayer?.position === "골키퍼"
                                  ? "무료 신청하기"
                                  : `${schedule.fee.toLocaleString()}원 신청하기`}
                          </button>
                        </>
                      );
                    })()}
