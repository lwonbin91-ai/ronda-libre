...
function VideoCard({ video }: { video: ScheduleVideo }) {
  const embed = getVideoEmbed(video.videoUrl);
  const isYoutube = embed?.type === "youtube";
  const date = new Date(video.scheduledAt);

  return (
    <div className="bg-[#0d0d0d] border border-white/6 rounded-2xl overflow-hidden hover:border-white/12 transition-all">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
              ...
              {video.type === "SEASON" ? "시즌 리그" : "오픈 매칭"}
            </span>
            {video.level && video.level !== "ALL" && (
        ...
        <p className="font-black text-sm leading-tight">{video.title}</p>
        {video.videoTitle && <p className="text-xs text-gray-500 mt-1">{video.videoTitle}</p>}
        {video.location && <p className="text-xs text-gray-600 mt-0.5">{video.location}</p>}
        <Link
          href={`/matches/${video.id}`}
...
export default function VideosPage() {
  const [videos, setVideos] = useState<ScheduleVideo[]>([]);
    ...
    <div className="max-w-6xl mx-auto px-6 py-14">
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="bg-[#0d0d0d] border border-white/5 rounded-2xl overflow-hidden animate-pulse">
